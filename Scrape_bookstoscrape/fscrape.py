...
def find_book(h3):
    ...
    link = a['href']
    link2 = link[8:]
    return "http://books.toscrape.com/catalogue" + link2
...
def scrape_page(url_book, information):
    ...
    if response.ok:
        ...
        list_data = [
            url_book, tds[0].text, title, tds[2].text, tds[3].text,
            tds[5].text, ptag[3].text, categorie[3].text, review2[1],
            url_book[36:-11], image_link[0]
            ]
        datawriter = csv.writer(information)
        datawriter.writerow(list_data)
        img_data = requests.get(image_link[0]).content
        with open(
            '../image_scrape/' + url_book[36:-11]
                + '.jpg', 'wb') as download:
